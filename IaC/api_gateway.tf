...
resource "aws_api_gateway_resource" "database-analytics" {
  path_part   = "database-analytics"
  parent_id   = aws_api_gateway_rest_api.database_toolkit.root_resource_id
  rest_api_id = aws_api_gateway_rest_api.database_toolkit.id
}
...
resource "aws_api_gateway_method" "database-analytics" {
  rest_api_id      = aws_api_gateway_rest_api.database_toolkit.id
  resource_id      = aws_api_gateway_resource.database-analytics.id
  http_method      = "GET"
  authorization    = "NONE"
  api_key_required = true
}
...
resource "aws_api_gateway_integration" "database-analytics" {
  rest_api_id             = aws_api_gateway_rest_api.database_toolkit.id
  resource_id             = aws_api_gateway_resource.database-analytics.id
  http_method             = aws_api_gateway_method.database-analytics.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.database-analytics.invoke_arn
}
...
resource "aws_lambda_permission" "database-analytics" {
  statement_id  = "AllowExecutionFromAPIGateway"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.database-analytics.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.database_toolkit.execution_arn}/*/*"
}
